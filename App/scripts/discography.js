...
export class Album {
	...
	constructor(name, cover, discs) {
		...
		if (cover === null) {
			this.cover = textures.T.cover_unknown;
		} else {
			this.cover = cover;
		}
...
export function updateEverything() {
	...
	for (const album of albums) {
		// Update album covers
		if (typeof album.cover === 'string' && textures.T[album.cover]) {
			album.cover = textures.T[album.cover];
		}
		album.update();
		...
		for (const disc of album.discs) {
			// Update disc covers
			if (typeof disc.cover === 'string' && textures.T[disc.cover]) {
				disc.cover = textures.T[disc.cover];
			}
			disc.update(album);

			for (const song of disc.songs) {
				// Update song covers
				if (typeof song.cover === 'string' && textures.T[song.cover]) {
					song.cover = textures.T[song.cover];
				}
				song.update();
				song.id[0] = songID;
				songID++;
...
export const albums = [
	...
	[
		...
		[
			...
			new Song(`David's Theme`, {
				...
				longDescription: `Here's an interesting fact. The reason this song takes a second to start is because when I first wrote it, I had a bitcrush effect at the end that faded out with the song (the same effect happens at the end of "Cade's Theme"). The effect would sound different depending on what measure it began, so I moved the start of the song over to get the sound I wanted. I couldn't be bothered to move it back after removing the effect months later.\n[Taken from MultiJumper Character Overview]\nDavid Andrews is a low-lying nobody from Earth. He lives in a small apartment on the outskirts of a major city. He often stays in his apartment, usually thinking about what his life could have been. His parents barely call him, so he lives a very lonely life. The first theme is calm and 8-bit. His second theme is nearly entirely composed of drums, incurring a state of panic and worry, along with referencing his hidden love of drums.\nTheme 1: "Rolling Down the Street, in my Katamari" by Fearofdark. Theme 2: "Drum and Drone" by Justin Hurwitzz. Extra Theme: "Savior of the Waking World" by Toby Fox.\n[Modern description starts here]\nDavid Andrews is a hasty conglomeration of many protagonist ideas I had for MultiJumper. At the start, the protagonist was me. It was going to be a self-insert novel about my adventures through video game worlds. After much ridicule from my friends, I changed his name to David Branson, which is just an inverted form of Brannon Davidson. A few months later, after having returned from a hiatus on MultiJumper, I realized it was a little on the nose, and changed his name to David Andrews; a combination of Davidson and Andrew Hussie. I hoped nobody would notice. David lives a life I had imagined myself to live as a child: alone in Kansas City, living a difficult and lonely life. For some reason that was the only life I could imagine my adult self living. I suppose I was inspired by the 2018 film "Venom" and the living conditions of the protagonist, Eddie Brock.\nDavid's journey in the first chapter of MultiJumper was the most I had ever actually written in text. His journey starts because he needs to buy a carton of milk from his local convenience store. He leaves his apartment and ventures into the pouring rain (I directly mimic this scene in the first chapter of TDCC, as Brannon experiences a dream where he leaves his hotel and sits on a bench in the rain). David makes it to the store, but two men burst into the store with guns, demanding that the cashier give them all the money in the register. One of the men finds David hiding behind a rack of water bottles and drags him to the entrance. David realizes how insane his life is at that moment and has a mental breakdown. He tries to stand up to the two men in a moment of stupidity, and the men are so amazed that they decide they'll kill him.\nThat's when a hooded figure appears and quickly dispatches the two criminals. Without any explanation, the figure takes David and guides him into the woods behind the store. David asks a thousand questions and the hooded figure barely explains. David tries to break free from the figure's grasp, but can't. Once they reach a secluded area, the man explains himself to be the Keeper of the Multiverse. He tells David to call him Denizen. David freaks out, naturally. Denizen explains that David is one of the most important people in the multiverse, and that he is fated for things beyond his wildest imaginations. Denizen leaves David to sit in the rain completely alone, when a bright object falls out of the sky and crashes beside David. David examines the white, flickering ball that warps space-time with its incredible power. He is overcome with an intense urge to touch the ball, and when he does, he is given immense power. For some reason, however, the power is not all granted at once, and David cannot comprehend his powers.`,
	...
	[
		...
		[
			...
			new Song(`Concierge Crackdown`, {
				alternativeNames: [`Escaping the Temple`],
				daw: `GB`, date: `12/18/2021`,
				...
				motifs: [
					[0, 9, `© Plants vs Zombies Theme`],
					[17, 19, `© MeGaLoVania`],
					[51, 63, `© Leap of Faith`],
					[55, 63, `Buildup`],
					[62.5, 64, `Radical Synths`],
					[64, 84, `© Disco Divermento Duo`],
					[72, 84, `© Leap of Faith`],
					[100, 108, `David's Theme`],
				],
			}),
