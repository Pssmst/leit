...
// Getting values of root variables `--value-info-width` and `--value-info-padding-horizontal` from style.css
export let css_infoWidth = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--value-info-width').trim());
export const css_infoPaddingHorizontal = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--value-info-padding-horizontal').trim());
...
export async function initSong(song) {
    ...
    state.audio.loadedSurroundingSongs = false;
