...
export function drawTrackCanvas() {
	...
	render.drawRect(cnv.trackCtx, waveformStartX, 0, waveformWidth, waveformHeight, colors.track.waveform.bg);

	// Gradient aligned to the waveform box (CSS pixels)
	const grad = cnv.trackCtx.createLinearGradient(0, 0, 0, waveformHeight);
	grad.addColorStop( 0, colors.track.waveform.gradient[0]);
	grad.addColorStop(.5, colors.track.waveform.gradient[1]);
	grad.addColorStop( 1, colors.track.waveform.gradient[2]);
	...
	cnv.trackCtx.strokeStyle = colors.track.primary;
	cnv.trackCtx.stroke();
	cnv.trackCtx.restore();

	// Draw the debug info on the waveform
	render.drawText(cnv.trackCtx, state.debug.structureString, { font: state.font.debug, x: 5, y: -state.font.size.default + 5, color: colors.track.primary, verticalSpacing: 1 });
	...
	// Background
	render.drawRect(cnv.trackCtx, ttStartX, 0, ttWidth, canvasHeight, colors.track.timeline.bg);

	// Inner timeline (no padding)
	render.drawRect(cnv.trackCtx, ttPlayerStartX, ttInnerStartY, ttInnerWidth, ttInnerHeight, colors.track.timeline.inner);

	// Measure bar (at the top)
	render.drawRect(cnv.trackCtx, ttPlayerStartX, ttInnerStartY, ttInnerWidth, ttMeasureBarHeight_Total, colors.track.timeline.measureBar.bg, {
		shadow: { inner: true, shadowColor: colors.track.timeline.measureBar.shadow, shadowBlur: 3, left: false, right: false }
	});
	render.drawBorder(cnv.trackCtx, ttPlayerStartX, ttInnerStartY, ttInnerWidth, ttMeasureBarHeight_Total, colors.track.timeline.measureBar.bg);

	// Draw track motif panel
	render.drawRect(cnv.trackCtx, ttInnerStartX, ttInnerStartY, ttPanelWidth, ttInnerHeight, colors.track.timeline.inner);
	...
	if (state.trackTimeline.motifPanel.scrollbarNeeded) {
		...
		// Scrollbar bg
		render.drawRect(cnv.trackCtx, ttPanelScrollbarStartX, ttInnerStartY, ttPanelScrollbarWidth, ttPanelHeight, colors.track.panel.scrollbar.bg);
		// Scrollbar fg
		render.drawRect(cnv.trackCtx, ttPanelScrollbarStartX, ttPanelScrollbarStartY, ttPanelScrollbarWidth, ttPanelScrollbarFGHeight, colors.track.panel.scrollbar.fg);
	}
	...
	for (const motifData of state.selectedSong.motifs) {
		...
		if (state.hovering.motifPanel.motif) {
			...
			render.drawRect(cnv.trackCtx, motif.panelX, motif.panelY, motif.width, motif.height, motif.colors.color, {
				shadow: { inner: false, shadowColor: colors.track.timeline.shadow, shadowBlur: 4 }
			});
		}

		const motifDisplayName = helpers.truncateString(motif.name, motif.width);
		render.drawText(cnv.trackCtx, motifDisplayName, { x: motif.panelX + motifOffset, y: motif.panelY + motifOffset, fontSize: state.font.size.default, color: motif.colors.text });
		
		motifY += state.font.size.default + motifOffset*3;
	}
	render.drawRect(cnv.trackCtx, ttStartX, 0, ttWidth, ttInnerStartY, colors.track.timeline.bg);
	render.drawRect(cnv.trackCtx, ttStartX, ttInnerEndY, ttWidth, canvasHeight - ttInnerEndY, colors.track.timeline.bg);
	...
	*/
	...
	function canDraw(kind, x, width = 0) {
		...
		if (state.debug.visuals[4]) {
			switch (kind) {
				...
				case "measureNumber":
					render.drawBorder(cnv.trackCtx, overlapState.lastRight[kind], measureNumberTextY, cfg.minSpacing || 0, state.font.size.default, colors.debug.hitboxes.unallowed);
					render.drawBorder(cnv.trackCtx, leftX, measureNumberTextY, rightX-leftX, state.font.size.default, colors.debug.hitboxes.self);
					break;
				
				case "timeSig":
					render.drawBorder(cnv.trackCtx, overlapState.lastRight[kind], ttTimeSigStartY, cfg.minSpacing || 0, state.font.size.timeSignature * 1.5, colors.debug.hitboxes.unallowed);
					render.drawBorder(cnv.trackCtx, leftX, ttTimeSigStartY, rightX-leftX, state.font.size.timeSignature * 1.5, colors.debug.hitboxes.self);
					break;
				
				case "measureLine":
					render.drawBorder(cnv.trackCtx, overlapState.lastRight[kind], ttMeasureBarEndY, cfg.minSpacing || 0, ttInnerHeight - ttMeasureBarHeight_Total, colors.debug.hitboxes.unallowed);
					render.drawLine(cnv.trackCtx, overlapState.lastRight[kind], ttMeasureBarEndY, overlapState.lastRight[kind], ttInnerEndY, colors.debug.hitboxes.self);
					break;
			}
		}
	...
	for (let measure = 1; measure <= state.selectedSong.totalMeasures; measure++) {
		...
		if (canDraw('measureNumber', ttCurrentDrawnX, drawTextWidth)) {
			render.drawText(cnv.trackCtx, measureText, { x: ttCurrentDrawnX, y: measureNumberTextY, scaleX: measureScaleX, color: colors.track.timeline.text });
		}

		///  MEASURE LINE  //////////////////////////////////////////////////////////////////

		if (canDraw('measureLine', ttCurrentDrawnX, 1)) {
			render.drawLine(cnv.trackCtx, ttCurrentDrawnX, ttMeasureBarEndY, ttCurrentDrawnX, canvasHeight - ttPadding, colors.track.timeline.lines.measure, 1);
		}
		...
		if (measure === 1 || thisSig !== prevSig) {
			...
			if (canDraw('timeSig', ttCurrentDrawnX, glyphWidthEst)) {
				render.drawText(cnv.trackCtx, `${glyph1}\n${glyph2}`, { font: state.font.timeSignature, fontSize: state.font.size.timeSignature, x: ttCurrentDrawnX, y: ttTimeSigStartY, verticalSpacing: .55, color: colors.track.timeline.text });
			}
		}
		...
		if (relativeBeatWidth >= drawConfig.beatLine.minSpacing) {
			for (let beat = 1; beat <= thisStructInfo.timeSignature[0]; beat++) {
				...
				render.drawLine(cnv.trackCtx, ttFakeCurrentDrawnX, ttMeasureBarEndY, ttFakeCurrentDrawnX, canvasHeight - ttPadding, colors.track.timeline.lines.beat, 1);
			}
		} else {
	...
	if (state.selectedSong.motifs.length > 0 && state.selectedSong.motifs[0][2] != '') {
		...
		for (const tcMotifData of state.selectedSong.motifs) {
			...
			render.drawRect(cnv.trackCtx, startX, y, ttMotifWidth, state.trackTimeline.motifPanel.motifHeight, motifObj.colors.color);

			// Draw the label
			const tcMotifDataDisplayName = helpers.truncateString(tcMotifData[2], ttMotifWidth - 2);
			render.drawText(cnv.trackCtx, tcMotifDataDisplayName, { x: startX + 2, y: y + (state.trackTimeline.motifPanel.motifHeight - state.font.size.default) / 2, fontSize: state.font.size.default, color: motifObj.colors.text });
		}
	}
	...
	render.drawLine(cnv.trackCtx, timelineCurrentX, ttPadding, timelineCurrentX, canvasHeight - ttPadding, colors.track.primary, 1);
}