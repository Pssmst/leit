			...
			for (const song of disc.songs) {
				...
				if (state.debug.visuals[3]) {
					if (audioCachePathList.includes(song.songPath)) {
						render.drawCircle(cnv.ctx, song.x + state.album.actualDimension / 10, song.y + state.album.actualDimension / 10, state.album.actualDimension / 8, colors.debug.loadedAudio.loaded, { strokeColor: colors.debug.loadedAudio.border });
					}
					else {
						render.drawCircle(cnv.ctx, song.x + state.album.actualDimension / 10, song.y + state.album.actualDimension / 10, state.album.actualDimension / 8, colors.debug.loadedAudio.unloaded, { strokeColor: colors.debug.loadedAudio.border});
					}
				}
